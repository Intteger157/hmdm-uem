...
import com.hmdm.launcher.task.ConfirmRemoteControlLaunchTask;
import com.hmdm.launcher.task.ConfirmRemoteControlStopTask;
...
import com.hmdm.launcher.util.RemoteControlHelper;
...
public class ConfigUpdater {
    ...
    private void checkRemoteControl() {
        ServerConfig config = settingsHelper != null ? settingsHelper.getConfig() : null;
        if (config != null && config.getRemoteControlStop() != null && config.getRemoteControlStop()) {
            RemoteLogger.log(context, Const.LOG_INFO, "Stopping remote control by server request");
            RemoteControlHelper.stopAgent(context);
            ConfirmRemoteControlStopTask confirmTask = new ConfirmRemoteControlStopTask(context) {
                @Override
                protected void onPostExecute(Integer result) {
                    setDefaultLauncher();
                }
        ...
        if (config != null && config.getRemoteControlStart() != null && config.getRemoteControlStart()) {
            RemoteLogger.log(context, Const.LOG_INFO, "Starting remote control by server request");
            if (RemoteControlHelper.startAgent(context, config)) {
                ConfirmRemoteControlLaunchTask confirmTask = new ConfirmRemoteControlLaunchTask(context) {
                    @Override
                    protected void onPostExecute(Integer result) {
                        if (result == null || result != Const.TASK_SUCCESS) {
                            RemoteLogger.log(context, Const.LOG_WARN, "Failed to confirm remote control launch on server");
                        }
                        setDefaultLauncher();
                    }
                };
                ...
                RemoteLogger.log(context, Const.LOG_WARN, "Remote control agent start failed");
            }
        }
                                ...
                                if (packageName != null) {
                                    ...
                                    if (BuildConfig.SYSTEM_PRIVILEGES || Utils.isDeviceOwner(context)) {
                                        ...
                                        if (packageName.equals(Const.APUPPET_PACKAGE_NAME)) {
                                            RemoteControlHelper.prepareAgent(context, appPermissionStrategy);
                                        }
                                    }
